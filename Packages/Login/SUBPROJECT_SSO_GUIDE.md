...
## 使用 admin-web 可视化创建（推荐）

后台 `http://localhost:3040` → **SSO → 子项目脚手架**（`/sso/subprojects`）

1. 填写项目名、`app_id`、前后端端口、OAuth scope 等
2. **保存配置**（存浏览器 localStorage）或 **保存并注册 SSO 客户端**（调 unit-auth API，自动回填 `client_id` / `secret`）
3. 下载：
   - `frontend-config.json` — 前端 SDK 配置
   - `backend-config.json` — BFF `server/config.json`
   - **完整脚手架 ZIP** — 含 `src/`、`server/`、`vite.config.ts`、`package.json` 等，解压到仓库根目录即可
