// Minimal BFF showcase for unitauthsdk: MountBFF + one protected route.
package main
...
func main() {
	...
	log.Printf("unitauthsdk_demo bff :%s", cfg.Port)
...
func loadConfig() config {
	cfg := config{
		Port:         envOr("PORT", "5560"),
		UnitAuthURL:  envOr("UNIT_AUTH_URL", "http://localhost:8080"),
		ClientID:     os.Getenv("CLIENT_ID"),
		ClientSecret: os.Getenv("CLIENT_SECRET"),
		RedirectURI:  envOr("REDIRECT_URI", "http://localhost:5179"),
		AppID:        envOr("APP_ID", "sso_unitauthsdk_demo"),
	}
