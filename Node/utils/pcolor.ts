...
// replaceAll 低版本node不支持！
const toBrowserColor = (color) => (input) => `<span style="color:${color}">${input}</span>`.replace(/\n/g, '<br />');
const toBrowserColorB = (color) => (input) =>
  `<span style="background:${color};"><span style="filter: invert(100%);color:${color}">${input}</span></span>`.replace(/\n/g, '<br />');
