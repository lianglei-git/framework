 ...
 * @example
// 主进程：不能覆盖console.log
        const logger = createLogger({ plugins: [plugins.client_logger()], prefix: '[主进程]' });
        logger.log('这么适用');

// 子进程：可以覆盖console.log
        const logger = createLogger({ plugins: [plugins.sendPrintBranch()], prefix: '[CT]', port: 'CT管道' });
...
function createLogger(options = {}) {
  ...
  function output(_type, _msg, _options = {}) {
    const anys = { type: _type, msg: _msg, options: Object.assign(other, _options, {prefix}) };
    ...
    const format = (isBrowser) => {
      ...
      if (options.timestamp) {
        ...
        return isBrowser
          ? `<span>${new Date().toLocaleString()} ${browserTag} ${browserColors.bold(msg)}</span> `
          : `${colors.dim(new Date().toLocaleString())} ${tag} ${msg}`;
      } else {
